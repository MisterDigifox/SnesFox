...
std::vector<std::string> makeDebugLines(
    ...
    lines.push_back("");
    if (!paused) {
        // Full-speed path: avoid heavy PPU diagnostics, long text, and per-line TTF blits.
    ...
    // ---- Critical diagnostics first ----
    {
        ...
        // VMADDR only (full VRAM scan was ~32k tests/frame in the UI hot path)
        {
            std::ostringstream oss;
...
int runPpuSnap(const std::string& romPath, uint64_t frames) {
    ...
    std::cerr << "  forcedBlank=" << (p.forcedBlank() ? 1 : 0)
              << "  bgMode=" << static_cast<int>(p.bgMode())
              << "  bg3PrioBit=" << (p.bg3Priority() ? 1 : 0)
              << "  tm=$" << hex8(p.tm())
              << " (BG1234|OBJ="
              << ((p.tm() >> 0) & 1) << ((p.tm() >> 1) & 1) << ((p.tm() >> 2) & 1)
              << ((p.tm() >> 3) & 1) << "|" << ((p.tm() >> 4) & 1) << ")\n";

    std::cerr << "  BG34NBA=$" << hex8(nba34) << " -> BG3 CHR word base $" << hexW(chrW) << '\n';

    unsigned tmPri13 = 0;   // BG tilemap priority is bit 13 (not bit 15)
    ...
    for (size_t i = 0; i < 256u * 224u; ++i) {
        ...
        if ((c >> 24) != 0xFF) continue;   // opaque only
        if (c != 0xFF000000u) {
            ++nonBlack;
            size_t si = uniq;
            bool   dup = false;
    ...
    std::cerr << "  heuristic: nonzero map words=" << tmNonzero
              << "  tile-priority(bit13) entries=" << tmPri13
    ...
    while (running) {
        ...
        if (!paused) {
            ...
            while ((cpu.cycles() - frameStartCycles) < CYCLES_PER_FRAME) {
                ...
                // Instruction log + disasm only while paused / single-step — updating every CPU
                // step here dominated frame time at full speed.
            }
        } else if (stepOnce) {
            stepOnce = false;
