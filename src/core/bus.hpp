...
public:
    ...
    // Debug-only: freezes the SPC700/S-DSP (ARAM/voices stop changing) while the main CPU/PPU
    // keep running, so the APU RAM/Voices debug views can be inspected as a static snapshot.
    void setApuPaused(bool paused) { m_apuPaused = paused; }
