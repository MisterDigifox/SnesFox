...
clang++ -std=c++20 src/game/main_game.cpp src/core/*.cpp src/macOS/*.mm tests/*.cpp imgui/*.cpp imgui/backends/*.cpp -o $ROM_NAME \
  -I. \
  -Iimgui \
  -Iimgui/backends \
...
if [ "$(uname -s)" = "Darwin" ] && command -v codesign >/dev/null 2>&1; then
  codesign --force -s - ./snesfoxGame 2>/dev/null || true
fi
