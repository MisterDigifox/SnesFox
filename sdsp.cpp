...
void Sdsp::mixOneSample(std::array<uint8_t, 65536>& aram) {
    ...
    for (size_t i = 0; i < kVoices; ++i) {
        ...
        // 4-point Catmull-Rom interpolation between the two samples straddling the
        // fractional position left over in the pitch counter, using one sample of
        // lookback/lookahead on each side. Real hardware uses a 4-tap Gaussian table;
        // this cubic is a from-scratch approximation with similar (better than linear)
        // high-frequency rolloff, which matters most for high-pitched voices where the
        // pitch counter skips several source samples per output tick and plain
        // nearest/linear resampling aliases into audible harsh/"saturated" distortion.
        ...
        const double t = static_cast<double>(v.pitchCounter) / 4096.0; // 0..1
        const double interp = 0.5 * ((2.0 * p1)
            + (-p0 + p2) * t
            + (2.0 * p0 - 5.0 * p1 + 4.0 * p2 - p3) * t * t
            + (-p0 + 3.0 * p1 - 3.0 * p2 + p3) * t * t * t);
        const int sample = static_cast<int>(interp);
