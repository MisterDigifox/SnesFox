...
void testPitchInterpolatesBetweenSamples() {
    ...
    dsp.runClocks(32, aram); // tick 0: pitchCounter at 0 -> exactly on sample 0 (value 0)
    Sdsp::PcmFrame f0{};
    dsp.popSamples(&f0, 1);

    expectTrue(f0.left > 0, "half-rate playback: first tick leaks a small amount from the next sample (off=0 isn't an ideal pass-through)");

    dsp.runClocks(32, aram); // tick 1: pitchCounter now at 0x800 -> halfway between sample 0 (0) and sample 1 (1536)
    Sdsp::PcmFrame f1{};
    dsp.popSamples(&f1, 1);
    // Halfway to sample 1, so this blends in noticeably more of it than tick 0 did, while
    // staying well below the ~1536-scale midpoint (envelope/volume-attenuated).
    expectTrue(f1.left > f0.left && f1.left < 1200,
        "half-rate playback: second tick (closer to sample 1) blends in more than the first tick did");
}
...
void testBrrFilterOverflowWrapsLikeHardware() {
    ...
    Sdsp::PcmFrame f1{};
    dsp.popSamples(&f1, 1);
    // At this tick's exact grid point (unity pitch -> frac=0 every tick), Gaussian
    // interpolation gives the wrapped sample (decoded[1], ~-10880) the dominant ~64% weight
    // but still leaks in ~18% of its large-positive neighbor (decoded[0], ~+28672) — see
    // gaussianTable()/gaussianInterpolate() in sdsp.cpp. So the aggregate lands small-and-
    // positive rather than strictly negative, but the *magnitude* still clearly distinguishes
    // "wrapped" from "naively saturated": substituting a hard +32767 clamp for the wrapped
    // sample would blend into a much larger aggregate (tens of thousands), not a small one.
    expectTrue(f1.left >= 0 && f1.left < 10000,
        "BRR filter overflow wraps to a small value (hardware), not a large one (naive +32767 saturation)");
}
