...
**Joypad auto-read is latched on every real VBlank edge, independent of NMI-enable** (`Bus::consumeVblankLatch()`, `snesfox_app.cpp`'s `advanceCpuScheduling`) — real hardware's `$4200` has NMI-enable (bit7) and auto-joypad-read as separate concerns, both driven by VBlank but not gated by each other. `stepPeripherals`'s own boolean return value stays gated by NMI-enable (its early-return interacts with the H-IRQ edge check right after it, which must not change) — `m_vblankLatchPending`/`consumeVblankLatch()` is a separate, additive flag set unconditionally on the same VBlank edge, consumed by the app loop to resample `sampleJoy1`/`sampleJoy2` regardless of whether NMI fires this call. Before this fix, input appeared to work fine on ordinary ROMs but felt completely dead on Super FX titles (`StarFox`/`StarFrog`/`Star3D`) — those routinely run long stretches with interrupts masked while polling GSU `SFR.GO`/H-V-counters (see the GSU section below), so `m_nmiEnabled` could stay false for most of a session, starving joypad sampling under the old NMI-gated logic even though real hardware keeps latching fresh input the whole time.
...
**Fixed (2026-08-19): DirectPage/Absolute(X/Y) addressing was missing the +1 cycle for 16-bit accesses.** `cpuOpcodesTable`'s `cyclesNumber` for these modes holds the conventional 8-bit-operand baseline (e.g. 4 for `dp,X`, matching every published 65816 cycle table), and real hardware adds exactly one more bus cycle whenever the access is 16 bits wide instead of 8 — confirmed against ares's `WDC65816` core, where e.g. `instructionDirectRead16`/`instructionBankWrite16` always issue exactly one more `read()`/`write()` call than their 8-bit counterparts. `CPU::step()` never added this extra cycle; `hasResolvableOperand`/`operandAccesses` (used a few lines below to weight `fineCycles`'s fetch/operand-speed split) already correctly computed "is this a 2-byte access" per-opcode (M-flag for most opcodes, X-flag for `LDX`/`LDY`/`STX`/`STY`/`CPX`/`CPY`) but that result was never fed back into the cycle *count* itself — only its internal split. Fixed by computing that width check before the `m_cycles`/`baseCycles` line and adding the extra cycle there too. Given how much native-mode (M=0) SNES code exists, this was a systemic, cumulative undercount — exactly the class of bug ("a fully deterministic polling loop never converges," "a GSU/CPU handshake barely stays synchronized") this codebase's other cycle-precision fixes above were chasing. Verified: `selftest` 22/22, `release.sh` round-trip unchanged, Star3D/StarFox GSU rendering both re-confirmed visually unaffected. Does **not** fully explain the still-open StarFrog Z-drift bug (see the `emulate-gsu-starfrog` skill) — applying it shifted that bug's exact timing (drift starts ~20 frames later) but didn't eliminate it, so there is likely at least one more timing or logic divergence still to find there.
